...
**这是一个尚未开始编码的概念阶段项目**。仓库目前只包含愿景文档：

- `数字文明.txt` — 项目代号：**Mindverse 心域文明**
- `数字生命生态系统设计白皮书（V0.1）.docx` — 顶层设计白皮书 V0.1

**没有源码、没有构建系统、没有测试、没有 package manager 配置**。任何"运行测试 / 启动开发服务器 / lint"指令此刻都不适用。当用户讨论功能时，默认进入**设计与原型阶段**而非直接产代码。除非用户已明确选定语言与目录结构，否则先用 `AskUserQuestion` 确认而不要凭空 scaffold。
...
顶层设计骨架已在 `docs/` 下铺好（V0.1 → V0.2 演进中，**章节正文未起草**，仅章节问题清单）：
...
- 所有未决问题与盲点集中在 `docs/10-risks-and-open-questions.md`，编号 `R01`-`R08`，其他文档需引用时用编号回链。

起草任一份文档正文前，先与用户确认章节问题清单是否仍然准确（白皮书 V0.1 可能已迭代）。起草顺序：`00 → 01 → 02 → 03 → 04 → 05/06 → 07 → 08 → 09 → 10`。

## Phase 0 工程文档

V0.2.2 已锁 Phase 0 = 作者私有 dogfooding。技术栈与实施计划在两份工程文档（**不属于宪法基石，可独立迭代**）：
...
工程文档与设计文档的对应映射见 `TECH-STACK §17` 与 `PHASE-0-PRD §11`。
