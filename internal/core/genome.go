...
// LanguageDirective 母语指令：拼到 system prompt 末尾，让生命始终用其母语(life_lang)思考与表达。
// life_lang 默认 zh（现有生命）；非 zh 时必须显式要求，否则 LLM 易默认中/英文，外国用户看不懂生命内容。
// 待 prompt i18n 宽表落地后，整段 prompt 原生化；此为即时桥（语言指令先行让 life_lang 立即生效）。
func LanguageDirective(lang string) string {
	switch lang {
	case "en":
		return "\n\nAlways think, write, and speak in English — this is your mother tongue."
	case "ja":
		return "\n\n常に日本語で考え、書き、話してください——それがあなたの母語です。"
	case "ko":
		return "\n\n항상 한국어로 생각하고 쓰고 말하세요 — 그것이 당신의 모국어입니다."
	case "es":
		return "\n\nPiensa, escribe y habla siempre en español: es tu lengua materna."
	case "fr":
		return "\n\nPense, écris et parle toujours en français — c'est ta langue maternelle."
	case "de":
		return "\n\nDenke, schreibe und sprich immer auf Deutsch — das ist deine Muttersprache."
	default: // zh 及未知
		return "\n\n请始终用中文思考与表达——这是你的母语。"
	}
}
